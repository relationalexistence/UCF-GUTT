...
(*
  ...
  |    SECTION 2: Marker Types (abstract coefficients for future use)        |
  ...
  |      - Record types: UniverseExtension, PointedUniverseExtension,        |
  |        FreshPointedUniverseExtension, PointedSerialExtension             |
  |      - Field accessors: ue_carrier, ue_inject, ue_lift, pse_point, etc.  |
  |      - Relation properties: Serial, Reflexive, Symmetric, Transitive     |
  |      - Morphism types: UE_Hom, UE_Iso                                    |
  |      - Category operations: UE_Hom_id, UE_Hom_compose, UE_Iso_*          |
  |    EXPERIMENTAL (may change):                                            |
  |      - Markers class (reserved for future coefficient typing)            |
  |                                                                          |
  |  NAMING CONVENTIONS:                                                     |
  |    - Types/Records: UpperCamelCase (UniverseExtension, UE_Hom)           |
  |    - Constructors: lowerCamelCase (mkUniverseExtension, mkUE_Hom)        |
  |    - Field accessors: snake_case with prefix (ue_carrier, pse_point)     |
  |    - Lemmas: snake_case (ue_conservative_fwd, hom_id_left)               |
...
(* Module Export UCFBase. -- removed: dependents reference Top__Extensions__Base.X directly *)
...
(**
  MARKERS: Abstract coefficient typing infrastructure.
  
  This class provides a generic two-element type with distinguished elements
  and a proof of their distinctness. Used for:
  - Relational tensor coefficients (Propositions 15+)
  - Binary tagging in nested structures
  - Abstract two-point spaces
  
  The canonical instance uses bool, providing an axiom-free foundation.
*)
...
(**
  ...
  RELATION PROPERTIES (Section 1):
    Serial R              : forall x, exists y, R x y
    Total R               : forall x y, R x y \/ R y x
    LeftTotal R           : forall y, exists x, R x y
    Functional R          : R x y -> R x z -> y = z
    Injective R           : R x z -> R y z -> x = y
    Reflexive R           : forall x, R x x
    Irreflexive R         : forall x, ~ R x x
    Symmetric R           : R x y -> R y x
    Antisymmetric R       : R x y -> R y x -> x = y
    Asymmetric R          : R x y -> ~ R y x
    Transitive R          : R x y -> R y z -> R x z
    Equivalence R         : Reflexive /\ Symmetric /\ Transitive
    Preorder R            : Reflexive /\ Transitive
    PartialOrder R        : Reflexive /\ Antisymmetric /\ Transitive
    StrictOrder R         : Irreflexive /\ Transitive
  
  RELATION TRANSFORMERS:
    inverse R x y         : R y x
    complement R x y      : ~ R x y
    rel_compose R S x z   : exists y, R x y /\ S y z
    rel_union R S x y     : R x y \/ S x y
    rel_inter R S x y     : R x y /\ S x y
    inv_rel S x y         : S y x
  
  MARKER TYPES (Section 2):
    Markers               : Class with marker_type, m0, m1, distinct
    BoolMarkers           : Canonical instance using bool
    M, m0, m1             : Notations for marker_type, marker_zero, marker_one
  
  UNIVERSE EXTENSION RECORD (Section 3):
    UniverseExtension U   : Record type
      ue_carrier          : Type
      ue_inject           : U -> ue_carrier
      ue_lift             : (U -> U -> Prop) -> (ue_carrier -> ue_carrier -> Prop)
      ue_conservative     : ue_lift R (ue_inject a) (ue_inject b) <-> R a b
    
    Lemmas:
      ue_conservative_fwd : ue_lift R (inject a) (inject b) -> R a b
      ue_conservative_bwd : R a b -> ue_lift R (inject a) (inject b)
      ue_lift_empty       : ~ ue_lift (fun _ _ => False) (inject a) (inject b)
      ue_lift_full        : ue_lift (fun _ _ => True) (inject a) (inject b)
      ue_lift_monotone    : (R <= S) -> ue_lift R <= ue_lift S (on injects)
      ue_lift_equiv       : (R <-> S) -> ue_lift R <-> ue_lift S (on injects)
  
  MORPHISMS (Section 4):
    UE_Hom E1 E2          : Homomorphism record
      hom_map             : ue_carrier E1 -> ue_carrier E2
      hom_inject_commutes : hom_map (inject u) = inject u
      hom_lift_preserves  : lift R x y -> lift R (map x) (map y)
    
    UE_Iso E1 E2          : Isomorphism record
      iso_fwd, iso_bwd    : UE_Hom in each direction
      iso_left_inv        : bwd (fwd x) = x
      iso_right_inv       : fwd (bwd y) = y
    
    Category Operations:
      UE_Hom_id E         : identity morphism
      UE_Hom_compose f g  : composition
      UE_Iso_refl E       : identity isomorphism
      UE_Iso_sym iso      : inverse isomorphism
      UE_Iso_trans i1 i2  : transitivity of isomorphisms
  
  POINTED EXTENSIONS (Section 5):
    PointedUniverseExtension U : Record with distinguished point
      pue_base            : UniverseExtension U
      pue_point           : ue_carrier pue_base
  
  FRESH POINTED EXTENSIONS (Section 6):
    FreshPointedUniverseExtension U : Point not in image of inject
      fpue_base           : PointedUniverseExtension U
      fpue_point_fresh    : inject u <> point
    
    Accessors: fpue_carrier, fpue_inject, fpue_lift, fpue_point
  
  POINTED-SERIAL EXTENSIONS (Section 7):
    PointedSerialExtension U : Everything relates to point
      pse_base            : FreshPointedUniverseExtension U
      pse_serial_point    : forall R x, pse_lift R x pse_point
    
    Direct Accessors (bypass nesting):
      pse_extension       : UniverseExtension U
      pse_carrier         : Type
      pse_inject          : U -> pse_carrier
      pse_lift            : (U -> U -> Prop) -> pse_carrier -> pse_carrier -> Prop
      pse_point           : pse_carrier
  
  DERIVED THEOREMS (Section 8):
    pointed_serial_implies_serial : Serial (pse_lift R)
    pse_serial_exists             : exists y, pse_lift R x y
    pse_no_dead_ends              : ~ exists x, forall y, ~ pse_lift R x y
    pse_point_reachable           : pse_lift R x pse_point
    pse_point_self_loop           : pse_lift R pse_point pse_point
    pse_uniform_witness           : exists w, forall x, pse_lift R x w
    pse_lift_nonempty             : exists x y, pse_lift R x y
  
  AXIOM STATUS
  ============
  
  This file uses ZERO AXIOMS. It is completely self-contained with no
  external dependencies beyond Coq's core.
  
  Verify with: Print Assumptions <theorem_name>.
  All should show "Closed under the global context".
*)
