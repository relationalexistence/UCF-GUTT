...
(*
  ...
  |      - Identity.id_extension                                             |
  |      - Composition.compose, >> notation                                  |
  |      - Composition.compose_id_left_iso, compose_id_right_iso             |
  |      - Composition.compose_assoc_iso                                     |
  |      - UniversalProperties.lift_restrict_roundtrip, lift_monotone        |
  |      - SerialComposition.iter_carrier, iter_inject, iter_lift            |
  |      - SerialComposition.double_completion, compose_pse                  |
  |    STABLE HINT DATABASES:                                                |
  |      - composition, serial_composition                                   |
  |                                                                          |
  |  NAMING CONVENTIONS:                                                     |
  |    - Module functions: snake_case (iter_carrier, compose_inject)         |
  |    - Isomorphisms: *_iso suffix (compose_id_left_iso)                    |
  |    - Homomorphisms: *_hom_fwd, *_hom_bwd for directions                  |
  |    - Properties: *_preserves_*, *_respects_*                             |
  |                                                                          |
...
Module Composition.
  ...
  (** Left unit law: id >> E ~ E (isomorphic) *)
  ...
  (** Right unit law: E >> id ~ E (isomorphic) *)
  ...
  (** Associativity: (E1 >> E2) >> E3 ~ E1 >> (E2 >> E3) (isomorphic) *)
  ...
  Section Functoriality.
    ...
    (** Composition respects morphisms: if we have a homomorphism f : E1 -> E1'
        and E2 is defined over the shared carrier, we get a morphism between
        the composed extensions. 
        
        NOTE: Full functoriality requires carrier equality or transport.
        Here we provide the case where carriers are definitionally equal. *)
    
    (** When E1 and E1' share the same carrier type, composition with E2 
        respects homomorphisms. *)
    Theorem compose_hom_right : forall 
        (E1 : UniverseExtension U)
        (E2 E2' : UniverseExtension (ue_carrier E1))
        (h : UE_Hom E2 E2'),
      UE_Hom (compose E1 E2) (compose E1 E2').
    Proof.
      intros E1 E2 E2' h.
      refine (@mkUE_Hom U (compose E1 E2) (compose E1 E2') (hom_map h) _ _).
      - intro u. simpl. apply hom_inject_commutes.
      - intros R x y H. simpl in *. apply hom_lift_preserves. exact H.
    Defined.

    (** Composition preserves isomorphisms on the right. *)
    Theorem compose_iso_right : forall 
        (E1 : UniverseExtension U)
        (E2 E2' : UniverseExtension (ue_carrier E1))
        (iso : UE_Iso E2 E2'),
    ...
    Proof.
      intros E1 E2 E2' iso.
      refine {|
        iso_fwd := compose_hom_right E1 E2 E2' (iso_fwd iso);
        iso_bwd := compose_hom_right E1 E2' E2 (iso_bwd iso)
      |}.
      - intro x. simpl. apply iso_left_inv.
      - intro y. simpl. apply iso_right_inv.
    Defined.
...
Module SerialComposition.
  ...
  *)
  ...
  Section GeneralComposition.
    ...
    (** Injection composes: u |-> inject2(inject1(u)). *)
    Definition compose_inject : U -> compose_carrier := ue_inject compose_ue.

    (** Lift composes: R |-> lift2(lift1(R)). *)
...
Print Assumptions Composition.compose_conservative.
