...
(*
  ...
  |      - WholeCompletion.carrier, inject, point, lift_rel                  |
  |      - WholeCompletion.serial, lift_conservative, point_terminal         |
  |      - WholeCompletion.as_extension, as_pointed_serial                   |
  |      - WholeCompletionHints lemmas                                       |
  |    STABLE TACTICS:                                                       |
  |      - wc_simpl, wc_auto                                                 |
  |    STABLE HINT DATABASE:                                                 |
  |      - whole_completion                                                  |
  |                                                                          |
  |  NAMING CONVENTIONS:                                                     |
  |    - Module functions: snake_case (lift_rel, carrier_case)               |
  |    - Lemmas: snake_case with descriptive names                           |
  |    - Direction suffixes: _fwd/_bwd for bidirectional lemmas              |
  |    - Record builders: as_* prefix (as_extension, as_pointed)             |
  |                                                                          |
...
(**
  ...
  CORE DEFINITIONS (WholeCompletion module):
    carrier U             : Type := option U
    inject u              : carrier U := Some u
    point                 : carrier U := None
    lift_rel R x y        : Prop (the serial completion of R)
  
  LIFT_REL SEMANTICS:
    lift_rel R (Some a) (Some b) = R a b      (conservative on U)
    lift_rel R x        None     = True       (everything -> Whole)
    lift_rel R None     (Some _) = False      (Whole is terminal)
    lift_rel R None     None     = True       (Whole self-loop)
  
  KEY LEMMAS:
    inject_injective      : inject a = inject b -> a = b
    point_fresh           : inject u <> point
    point_fresh_sym       : point <> inject u
    lift_conservative     : lift_rel R (inject a) (inject b) <-> R a b
    lift_conservative_fwd : lift_rel R (inject a) (inject b) -> R a b
    lift_conservative_bwd : R a b -> lift_rel R (inject a) (inject b)
    serial                : forall R x, lift_rel R x point
    weak_serial           : Serial (lift_rel R)
    point_self_loop       : lift_rel R point point
    point_terminal        : ~ lift_rel R point (inject u)
    lift_preserves        : R a b -> lift_rel R (inject a) (inject b)
    no_dead_ends_in_completion : ~ exists x, forall y, ~ lift_rel R x y
  
  INVERSION PRINCIPLES:
    carrier_case          : x = point \/ exists u, x = inject u
    lift_rel_to_elem_inv  : lift_rel R x (inject b) -> exists a, x = inject a /\ R a b
    lift_rel_from_point_inv : lift_rel R point y -> y = point
    carrier_eq_dec        : (decidable U) -> decidable (carrier U)
  
  PROPERTY PRESERVATION:
    lift_preserves_reflexive   : Reflexive R -> Reflexive (lift_rel R)
    lift_symmetric_on_elems    : Symmetric R -> (on elements) symmetric
    lift_transitive_on_elems   : Transitive R -> (on elements) transitive
    lift_trans_to_point        : chains ending at point work
    lift_monotone              : (R <= S) -> lift_rel R <= lift_rel S (on U)
    lift_monotone_general      : (R <= S) -> lift_rel R <= lift_rel S (all)
    lift_equiv                 : (R <-> S) -> lift_rel R <-> lift_rel S
    lift_inter                 : lift (R /\ S) <-> lift R /\ lift S (on U)
    lift_union                 : lift (R \/ S) <-> lift R \/ lift S (on U)
  
  RECORD INSTANCES:
    as_extension U        : UniverseExtension U
    as_pointed U          : PointedUniverseExtension U
    as_fresh_pointed U    : FreshPointedUniverseExtension U
    as_pointed_serial U   : PointedSerialExtension U
  
  ADDITIONAL THEOREMS:
    completion_minimality     : ~Serial R -> Serial (lift_rel R)
    point_unique_universal    : (forall R x, lift R x w) -> w = point
    inject_reflects           : lift_rel R (inject a) (inject b) <-> R a b
    carrier_disjoint_union    : x from U or x = point
    universal_relation_to_point : lift_rel R x point
    point_only_reaches_point  : lift_rel R point y -> y = point
  
  HINT DATABASE (whole_completion):
    Rewrite: lift_between_elems
    Resolve: lift_to_whole, lift_whole_whole, lift_elem_to_whole,
             lift_preserves, serial, point_self_loop
    Extern:  point_terminal
  
  TACTICS:
    wc_simpl              : unfold lift_rel, inject, point; simpl
    wc_auto               : auto with whole_completion + wc_simpl
  
  AXIOM STATUS
  ============
  
  This file uses ZERO AXIOMS. All constructions are purely definitional.
  
  Verify with: Print Assumptions WholeCompletion.serial.
