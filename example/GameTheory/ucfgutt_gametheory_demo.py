...
REQUIRED_W = ("uF_own", "uF_partner", "v", "Lf", "M0")
REQUIRED_C = ("coop", "dev", "sucker", "safe")

def _invalid(name, msg):
    print("═" * 62)
    print(" %s" % name)
    print(" INVALID INPUT: %s" % msg)

def run(name, spec):
    # ── input validation: every malformed spec gets a clean
    #    message, never a traceback ──────────────────────────
    if not isinstance(spec.get("W"), dict):
        return _invalid(name, "missing mechanism object 'W'")
    missing = [k for k in REQUIRED_W if k not in spec["W"]]
    if missing:
        return _invalid(name,
            "mechanism 'W' missing field(s): %s" % ", ".join(missing))
    if "q" not in spec:
        return _invalid(name, "missing discount 'q'")
    P_in = spec.get("P", DEFAULT_GAME)
    for i in (0, 1):
        if i not in P_in and str(i) not in P_in:
            return _invalid(name, "game 'P' missing player %d" % i)
    P_in = {int(k): v for k, v in P_in.items()}         if all(isinstance(k, str) for k in P_in) else P_in
    for i in (0, 1):
        miss = [c for c in REQUIRED_C if c not in P_in[i]]
        if miss:
            return _invalid(name,
                "player %d missing cell(s): %s" % (i, ", ".join(miss)))
    try:
        W = {k: Q(spec["W"][k]) for k in REQUIRED_W}
        P = {i: {c: Q(P_in[i][c]) for c in REQUIRED_C}
             for i in (0, 1)}
        q_probe = Q(spec["q"])
    except (ValueError, ZeroDivisionError, TypeError) as e:
        return _invalid(name,
            "value not an exact rational (like \"7/2\"): %s" % e)
    T_raw = spec["T"]
