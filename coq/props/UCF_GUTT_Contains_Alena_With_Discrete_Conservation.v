...
Module Concrete4D.
  ...
  Definition g : T2 := fun i j =>
    match i, j with
    | I0, I0 => -1 | I1, I1 => 1 | I2, I2 => 1 | I3, I3 => 1 | _, _ => 0
    end.
  ...
  Theorem A2_coupling_scalar_4D : xi_inv = k_quarter * (trace_ginv H).
  Proof. unfold xi_inv. now rewrite A1_induced_metric_4D. Qed.

  Theorem A3_scalar_invariant_defn_4D : Lambda_rho = lambda_const * F2.
  Proof. reflexivity. Qed.
...
Module DiscreteConservationFD.
  ...
  (* === CONCRETE DEFINITIONS (no hypotheses!) === *)
  
  (* Concrete constant vector field *)
  Definition Uvec_concrete : V1F := fun p i => 
    match i with I0 => 1 | I1 => 0 | I2 => 0 | I3 => 0 end.
...
Qed.
  ...
  (* === CONSERVATION THEOREM (proven for concrete fields) === *)
  
  Parameters (rho c2 xi : R).

  Definition matter : T2F := fun p => scaleC rho (outerF Uvec_concrete Uvec_concrete) p.
  Definition pressure : R := (c2 * rho).
  Definition geom : T2F := fun p i j => g_concrete p i j - xi * H_concrete p i j.
  ...
  Theorem A5_conservation_fd_concrete : forall p alpha, Div Tfield p alpha = 0.
  Proof.
    ...
    { intro b. unfold Tfield, DbT.
      ...
      unfold DbT in Hm, Hg. apply minus0_eq in Hm. apply minus0_eq in Hg.
      rewrite Hm, Hg. now apply Rminus_diag_eq.
    }
    rewrite (Hz I0), (Hz I1), (Hz I2), (Hz I3).
    repeat rewrite Rplus_0_l. repeat rewrite Rplus_0_r. reflexivity.
  Qed.

End DiscreteConservationFD.
...
Include DiscreteConservationFD_Var.DiscreteConservationFD_Var_Derived.
  ...
  Proof.
    intros b a p0 Hb0.
    split.
    - (* First part: I_local (s_step b a) p0 = 0 *)
      rewrite I_local_closed_form.
      unfold s_step. rewrite Hb0. simpl. ring.
    - (* Second part: I_nonlocal b (s_step b a) p0 = a*a *)
      unfold I_nonlocal.
      (* Simplify I_local (s_step b a) p0 *)
      assert (H1 : I_local (s_step b a) p0 = 0).
      { rewrite I_local_closed_form. unfold s_step. rewrite Hb0. simpl. ring. }
      rewrite H1.
      (* Simplify I_local (s_step b a) (flip b p0) *)
      assert (Hflip : (flip b p0) b = Z1) by (apply flip_b_Z0; exact Hb0).
      assert (H2 : I_local (s_step b a) (flip b p0) = 2 * (a*a)).
      { rewrite I_local_closed_form. unfold s_step. rewrite Hflip. simpl. ring. }
      rewrite H2.
